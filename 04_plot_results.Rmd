...
df_bc = read_delim("results/simulated_new/bc.tsv")
...
df_bc_long <- df_bc |>
    ...
    mutate(method = "Mean Bray-Curtis")
df_lordist_long <- df_lordist |>
    select(fval, pval, output, requested_sparsity, achieved_sparsity) |>
    mutate(method = "LorDist")
df_long <- rbind(
    df_ctf_sample_long, df_ctf_subject_long, df_lordist_long, df_bc_long
) |>
    mutate(requested_sparsity = as.factor(requested_sparsity))
df_long |> head()
```

Make boxplots comparing p values
...
df_plot <- df_long |>
    ...
    summarise(
        median_pval = median(pval), median_fval = median(fval),
