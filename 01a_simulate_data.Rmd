...
```{r}
# This is a slightly wasteful way of doing it on reflection - could just
# plot the values across continuous time on the xaxis.
df_one <- seq(1:100) |>
    map(\(x) {
...
df_long <- rbind(
    df_one |> mutate(taxon = "One - Control"),
    df_one |> mutate(taxon = "One - Case"),
    df_two_ctrl, df_two_case, df_three_case, df_three_ctrl) |>
    mutate(
        timepoint_num = gsub("V", "", timepoint) |> as.numeric(),
        phenotype = map_chr(taxon, function(x) {
            if(grepl("Control", x)) {return("Control")}
            if(grepl("Case", x)) {return("Case")}
            return("Error")
        }),
        phenotype = fct_relevel(phenotype, "Control", "Case")
    )
df_long
```
...
plt_taxa <- ggplot(
    ...
    guides(
        color = "none",
        linetype = guide_legend(title="Taxon")
    ) +
    theme_pubr()
    ggsave(
        "results/simulated/function_check.png",
        width = 6, 
        height = 3,
        scale = 1.5
    )
...
As an intuitive check, we can visualise the abundance of each taxon in 
each participant across time, and check that it is following one of the
expected functions

```{r}
plt_taxa <- example$matrix |>
    rownames_to_column("FeatureID") |>
    pivot_longer(-FeatureID, names_to="SampleID") |>
    left_join(example$metadata, join_by(SampleID == SampleID)) |>
    ggplot(
        aes(x = Timepoint, y = value)
    ) +
    geom_line(
        aes(group = SubjectID, color = Phenotype), linewidth=0.05, alpha=0.5
    ) +
    geom_smooth(
        aes(color = Phenotype)
    ) +
...
ggsave("results/simulated/taxa_plots.png", dpi=300, width=11, height=21)
```

These plots all look correct - they follow one of the defined functions.
...
### Very high sparsity simulation
Simulate datasets at very high sparsity for visualisation of Bray-Curtis and
LorDist performance

```{r}
set.seed(4298)
root_data <- get_dataset()
pth_sim_base <- file.path("data", "simulated_high")
for(s in c(0.8, 0.9, 0.95)) {
  smat <- random_sparse(root_data$matrix, s)
  dir_sim <- file.path(pth_sim_base, glue("sparsity_{s}"))
  pth_sim <- file.path(dir_sim, "1.tsv")
  dir.create(dir_sim, recursive=TRUE)
  write.table(smat$matrix, pth_sim, sep = "\t")
}
root_data$metadata |>
  write_delim(file.path(pth_sim_base, "sample_metadata.tsv"))
```